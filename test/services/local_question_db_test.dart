...
void main() {
  ...
  group('LocalQuestionDB.getQuestionsForLesson', () {
    ...
    test(
      ...
      () async {
        final result = await LocalQuestionDB.instance
            .getQuestionsForLesson('ac_st1', 'ac_s1_ses9_l1_does_not_exist', count: 5);
    ...
    test(
      ...
      () async {
        final result = await LocalQuestionDB.instance
            .getQuestionsForLesson('ac_st1', 'default', count: 20);
        await expectStagePrefixes(result);
      },
      timeout: const Timeout(Duration(minutes: 2)),
    );
    ...
    test(
      ...
      () async {
        ...
        final first = await db.getQuestionsForLesson('ac_st1', 'default', count: 20);
        final second = await db.getQuestionsForLesson('ac_st1', 'default', count: 20);
    ...
    test(
      ...
      () async {
        final result = await LocalQuestionDB.instance
            .getQuestionsForLesson('ac_st1', 'ac_s1_ses1_l1', count: 5);
        expect(result, isNotEmpty);
        expect(result.length, 5);
    ...
    test('unknown stage returns empty list', () async {
      final result = await LocalQuestionDB.instance
          .getQuestionsForLesson('unknown_stage', 'unknown_lesson', count: 10);
      expect(result, isEmpty);
    });

    test('stage fallback ids all belong to the stage prefix', () async {
      final db = LocalQuestionDB.instance;
      final result = await db.getQuestionsForLesson('ac_st1', 'no_such_lesson_zzz', count: 8);
      expect(result, isNotEmpty);
