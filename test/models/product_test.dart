...
void main() {
  group('Product.isBundle', () {
    test('returns false for donation-only products', () {
      expect(
        _testProducts.firstWhere((p) => p.id == 'donation_5').isBundle,
        isFalse,
      );
    });
    ...
    test('returns true for bundle products', () {
      expect(
        _testProducts.firstWhere((p) => p.id == 'bundle_protector').isBundle,
        isTrue,
      );
    });
  });
  ...
  group('Product.hasStreakProtector', () {
    ...
    test('returns true for bundle_protector', () {
      expect(
        _testProducts
            .firstWhere((p) => p.id == 'bundle_protector')
            .hasStreakProtector,
        isTrue,
      );
    });
