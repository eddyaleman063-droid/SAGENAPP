...
class ReauthenticationScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ReauthenticationScreen> createState() => _ReauthenticationScreenState();
}

class _ReauthenticationScreenState extends ConsumerState<ReauthenticationScreen> {
    ...
    if (_isOAuthUser) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(AppLocalizations.of(context)!.settingsDeleteAccountConfirm),
          actions: [
            Semantics(button: true, label: AppLocalizations.of(context)!.cancel, child: TextButton(
              onPressed: () => context.pop(false),
              child: Text(AppLocalizations.of(context)!.cancel),
            )),
            Semantics(button: true, label: AppLocalizations.of(context)!.settingsDeleteAccount, child: TextButton(
      ...
      try {
        ...
        if (!mounted) return;
final errorCode = e is AuthException ? e.code : ref.read(authProvider).errorMessage ?? 'unknown';
        SagenNotification.show(context, message: AuthException(errorCode).localizedMessage(l), type: NotificationType.error);
      ...
      if (currentAuth.errorMessage != null) {
        SagenNotification.show(context, message: AuthException(currentAuth.errorMessage!).localizedMessage(l), type: NotificationType.error);
      }
      setState(() => _isSubmitting = false);
      return;
    }
    ...
    try {
      await authNotifier.deleteAccount(email: auth.email, password: _passwordCtrl.text);
    } catch (e) {
      if (!mounted) return;
      final errorCode = e is AuthException ? e.code : ref.read(authProvider).errorMessage ?? 'unknown';
      if (errorCode == 'requires_recent_login') {
        SagenNotification.show(context, message: l.reauthWrongPassword, type: NotificationType.error);
      } else {
        SagenNotification.show(context, message: AuthException(errorCode).localizedMessage(l), type: NotificationType.error);
      }
      setState(() => _isSubmitting = false);
    ...
    if (currentAuth.errorMessage != null) {
      ...
      if (error == 'wrong_password' || error == 'invalid_credential') {
        ...
        SagenNotification.show(context, message: AuthException(error!).localizedMessage(l), type: NotificationType.error);
      }
    }
    if (!mounted) return;
    setState(() => _isSubmitting = false);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          l.reauthTitle,
          style: AppTextStyle.title.copyWith(color: context.textPrimary, fontWeight: FontWeight.w600),
        ),
      ),
      body: Container(
                ...
                child: Column(
                  ...
                  children: [
                    const SizedBox(height: AppSpacing.xxl),
                      const Icon(Icons.shield_rounded, size: 48, color: PremiumColors.primaryAccent),
                    ...
                    Semantics(label: l.authEmailLabel, child: TextFormField(
                      ...
                      decoration: InputDecoration(
                        ...
                        labelStyle: AppTextStyle.bodyMd.copyWith(color: context.textTertiary),
                    ...
                    const SizedBox(height: AppSpacing.lg),
                    if (!_isOAuthUser) ...[
                      Semantics(label: l.authPasswordLabel, child: TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: Semantics(button: true, label: _obscurePassword ? l.showPassword : l.hidePassword, child: IconButton(
                            icon: Icon(
                              ...
                              size: 20, color: PremiumColors.primaryAccent,
                            ),
                    ...
                    SizedBox(
                      ...
                      height: 52,
                      child: Semantics(button: true, label: l.reauthConfirm, child: ElevatedButton(
                        ...
                        child: _isSubmitting
                            ? const SizedBox(
                                width: 22, height: 22,
                                child: PremiumLoader(loading: true, child: SizedBox.expand()),
                              )
                            : Text(
                                l.reauthConfirm,
                    ...
                    SizedBox(
                      width: double.infinity,
                      height: 52,
                      child: Semantics(button: true, label: l.cancel, child: OutlinedButton(
                        onPressed: () {
                          ref.read(experienceServiceProvider).lightHaptic();
                          context.pop();
                        },
                        ...
                        child: Text(l.cancel, style: AppTextStyle.body.copyWith(fontWeight: FontWeight.w600)),
                      )),
                    ),
                  ],
